...
class Encoder(Chain):
    ...
    def __call__(self, x, enc=False, test=False):
        ...
        h = self.resconv0(h, enc)
        self.hiddens.append(h)
        h = self.resconv1(h, enc)
        h = F.max_pooling_2d(h, (2, 2))  # 32 -> 16
        self.hiddens.append(h)

        h = self.resconv2(h, enc)
        self.hiddens.append(h)
        h = self.resconv3(h, enc)
        h = F.max_pooling_2d(h, (2, 2))  # 16 -> 8
        self.hiddens.append(h)

        h = self.resconv4(h, enc)
        self.hiddens.append(h)
        h = self.resconv5(h, enc)
