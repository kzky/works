...
from meta_recon.losses import ReconstructionLoss, LSGANLoss, GANLoss, EntropyRegularizationLoss
from sklearn.metrics import confusion_matrix

class Experiment000(object):
    """Enc-MLP-Dec-Dis

    - Spatial size is reduce by strided convolution.
    """
    def __init__(self, device=None, learning_rate=1e-3, act=F.relu, n_cls=10):
        ...
        # Model
        from meta_recon.svhn.cnn_model_000 \
...
class Experiment001(Experiment000):
    ...
    def __init__(self, device=None, learning_rate=1e-3, act=F.relu, n_cls=10):
        ...
        # Model
        from meta_recon.svhn.cnn_model_001 \
            import Encoder, MLP, Decoder, Discriminator
        self.encoder = Encoder(device, act)
        self.mlp = MLP(device, act)
        self.decoder = Decoder(device, act)
