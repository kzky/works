...
class Experiment002(Experiment):
    """Experiment takes responsibility for a batch not for train-loop.
    """
    ...
    def forward_for_losses(self, x_l, y_l, x_u, test=False):
        ...
        for t in range(self.T):
            ...
            # Reconstruction for (x_l, )
            recon_loss_l = 0
            if t > 1:  # RC for enc(t+1) and dec(t)
                recon_loss_l = self.recon_loss(
                    None, None, self.mlp_enc.hiddens, mlp_dec_l_hiddens)
            x_l_recon = self.mlp_dec(y, test)
            mlp_dec_l_hiddens = self.mlp_dec.hiddens
            recon_loss_l += self.recon_loss(
                x_l_recon, x_l_recon_t0,  None, None)  # Virtual AE
            x_l_recon_t0 = x_l_recon
            if t == self.T - 1:
                self.mlp_enc(x_l_recon_t0, test)
                recon_loss_l += self.recon_loss(
                    None, None, self.mlp_enc.hiddens, self.mlp_dec.hiddens)
            recon_l_losses.append(recon_loss_l)
            ...
            recon_loss_u = 0
            if t > 1:  # RC for enc(t+1) and dec(t)
                recon_loss_u = self.recon_loss(
                    None, None, self.mlp_enc.hiddens, mlp_dec_u_hiddens)
            x_u_recon = self.mlp_dec(y, test)
            mlp_dec_u_hiddens = self.mlp_dec.hiddens
            recon_loss_u += self.recon_loss(
                x_u_recon, x_u_recon_t0,  None, None)  # Virtual AE
            x_u_recon_t0 = x_u_recon
            
            if t == self.T - 1:
                self.mlp_enc(x_u_recon_t0, test)
                recon_loss_u += self.recon_loss(
                    None, None, self.mlp_enc.hiddens, self.mlp_dec.hiddens)
            recon_u_losses.append(recon_loss_u)
...
class Experiment017(Experiment):
    """Experiment takes responsibility for a batch not for train-loop.
    """
...
Experiment004 = Experiment002
Experiment006 = Experiment002
Experiment008 = Experiment002

Experiment018 = Experiment017
Experiment022 = Experiment017
Experiment021 = Experiment019
Experiment023 = Experiment019
Experiment025 = Experiment019
