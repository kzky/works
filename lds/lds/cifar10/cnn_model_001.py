...
class Encoder(Chain):
    ...
    def __init__(self, act=F.relu):
        super(Encoder, self).__init__(
            ...
            bn_conv5=L.BatchNormalization(256, decay=0.9, use_cudnn=True),
    ...
    def __call__(self, x, test=False):
        ...
        h = self.conv5(h)
        h = self.bn_conv5(h, test)
        h = self.act(h)
        self.hiddens.append(h)
        h = F.max_pooling_2d(h, (2, 2))  # 8x8 -> 4x4
...
class Decoder(Chain):
    ...
    def __init__(self, act=F.relu):
        super(Decoder, self).__init__(
            ...
            bn_deconv6=L.BatchNormalization(64, decay=0.9, use_cudnn=True),
    ...
    def __call__(self, y, test=False):
        ...
        h = self.deconv5(h)  # 16x16 -> 32x32
        h = self.bn_deconv5(h, test)
        h = self.act(h)
        self.hiddens.append(h)

        h = self.deconv6(h)
        h = self.bn_deconv6(h, test)
        h = self.act(h)
        self.hiddens.append(h)
