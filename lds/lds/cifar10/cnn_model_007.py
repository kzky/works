...
class ConvResUnit(Chain):
    def __init__(self, maps, act=F.relu):
        super(ConvResUnit, self).__init__(
            ...
            conv2=L.Convolution2D(maps, maps, 3, stride=1, pad=1),
            bn2=L.BatchNormalization(maps, decay=0.9, use_cudnn=True),
        )
        self.act = act
...
class Encoder(Chain):
    ...
    def __init__(self, act=F.relu):
        super(Encoder, self).__init__(
            conv0=L.Convolution2D(3, 16, 3, stride=1, pad=1),
            bn0=L.BatchNormalization(16, decay=0.9, use_cudnn=True),
            block0=ConvResUnitPoolFinetune(16, act),
...
class DeconvResUnit(Chain):
    ...
    def __init__(self, maps, act=F.relu):
        super(DeconvResUnit, self).__init__(
            ...
            bn1=L.BatchNormalization(maps, decay=0.9, use_cudnn=True),
            deconv2=L.Convolution2D(maps, maps, 3, stride=1, pad=1),
            bn2=L.BatchNormalization(maps, decay=0.9, use_cudnn=True),
        )
        self.act = act
...
class Decoder(Chain):
    ...
    def __init__(self, act=F.relu):
        super(Decoder, self).__init__(
            ...
            deconv=L.Deconvolution2D(16, 3, 3, stride=1, pad=1),
        )
